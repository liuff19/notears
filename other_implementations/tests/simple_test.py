...
def simple_test(num_nodes, num_edges, verbose=False, cov=False):
    tam, tg = notears.utils.generate_random_dag(
        num_nodes, num_edges, probabilistic=False, edge_coefficient_range=[0.5, 2.0])
    data = notears.utils.simulate_from_dag_lg(tam, 1000)

    if not cov:
        nt_weight = notears.run(notears.notears_standard, data, notears.loss.least_squares_loss,
                                notears.loss.least_squares_loss_grad, e=1e-8, verbose=verbose)['W']
    else:
        nt_weight = notears.run(notears.notears_standard, data, notears.loss.least_squares_loss_cov,
                                notears.loss.least_squares_loss_cov_grad, e=1e-8, verbose=verbose)['W']
    nt_eam = notears.utils.threshold_output(
        nt_weight, nx.number_of_edges(tg), verbose=verbose)
